...
const filteredComponents = computed(() => {
  return loadedComponents.value.filter((item) => {
    ...
    // match keywords
    if (keywords[item.name]?.some((keyword) => keyword.toLowerCase().indexOf(text) !== -1)) return true;
...
<template>
  <Toast
    class="fixed select-none bottom-4 right-4 flex flex-col gap-4 items-right text-sm"
    v-model="toast"
    v-slot="{ value }"
  >
    <div :class="`bg-success text-background px-2 py-1 rounded shrink-0`">
      {{ value }}
    </div>
  </Toast>
  ...
  <div class="grid grid-cols-1 gap-4">
    ...
    <div class="border border-secondary rounded-lg p-4 grid grid-cols-[repeat(auto-fill,minmax(8rem,1fr))]">
      <button
        ...
        @click="
          copyToClipboard(item.componentName);
          toast.push('Copied');
        "
      >
